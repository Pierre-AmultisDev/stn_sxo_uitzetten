...
with open("corsa_qry_parameters.json", 'r') as parametersfile:
...
# info from corsa: KINGcode corsa_dossiercode
corsa_file_path = parameters.get("filepaths", {}).get("corsa_file_path", "./corsa/")
corsa_files_list = parameters.get("filepaths", {}).get("corsa_files_list", [])
                    ...
                    for corsafile in corsa_files_list:
                        if os.path.isfile(corsafile):
                            ...
                            for check_column in list_of_columns_to_check:
                                df_all = df_all.merge(df_corsa, how="left", left_on=check_column, right_on="KING_Zaakidentificatie")    
